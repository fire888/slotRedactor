...
export class HelperSortData {
    constructor (gameContextLink, config) {
        gameContext = gameContextLink
        //SYMBOLS = gameContext.components['slotMachine'].config.config.SYMBOLS // !!!!!!! change
        SYMBOLS = config.config.SYMBOLS // !!!!!!! change
...
const getRandomNumberNotInWithoutIdle = (arr1, arr2) => {
    let num = Math.floor(Math.random() * SYMBOLS.length)

    let isNumCanIn = true
    for (let i = 0; i < arr1.length; ++i) {
        arr1[i] === num && (isNumCanIn = false)
    }
    for (let i = 0; i < arr2.length; ++i) {
        arr2[i] === num && (isNumCanIn = false)
    }

    return isNumCanIn ? num : getRandomNumberNotInWithoutIdle(arr1, arr2)
}

const arrIdleAnimatedSymbols = []
const prepareIdleAnimatedSymbols = symbolsData => {
    for (let i = 0; i < symbolsData.length; ++i) {
        if (symbolsData[i].modeNormalAnimation && symbolsData[i].modeNormalAnimation === 'spineNormal') {
            arrIdleAnimatedSymbols.push(i)
        }
    }
}
...
export const prepareFinishSymbols = resp => {
    ...
    !arrIdleAnimatedSymbols.length && prepareIdleAnimatedSymbols(SYMBOLS)

    let symbolsTypes = []

    for (let i = 0; i < resp.wheels.length; ++i) {
        const column = resp.wheels[i].map(num => num < len ? num : Math.floor(num % len))
        symbolsTypes.push([getRandomNumberNotInWithoutIdle(column, arrIdleAnimatedSymbols)].concat(column))
    }
    return symbolsTypes
}
