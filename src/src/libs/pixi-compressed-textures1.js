/**
 * Created by Vasilii on 13.03.2021.
 */
...
(function (pixi_compressed_textures) {
    function extensionChooser(supportedExtensions) {
        ...
        return function (resource, next) {
            ...
            var filePureExtension = url.substr(url.lastIndexOf('.') + 1);
            ...
            for (var i = ext.length - 1; i >= 0; i--) {
                ...
                for (var j = 0; j < supportedExtensions.length; j++) {
                    if (ext[i] === supportedExtensions[j]) {
                        var extPureExtension = ext[i].substr(ext[i].lastIndexOf('.') + 1);
                        if (filePureExtension === 'jpg' || filePureExtension === 'png') {
                            if (extPureExtension === filePureExtension) {
